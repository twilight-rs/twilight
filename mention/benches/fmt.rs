...
fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function("format channel id", |b| {
        ...
        let formatter = Id::<ChannelMarker>::new_checked(999_999_999_999_999_999).mention();

        b.iter(|| format_id(&mut string, &formatter))
    });
    c.bench_function("format emoji id", |b| {
        let mut string = String::new();
        let formatter = Id::<EmojiMarker>::new_checked(999_999_999_999_999_999).mention();

        b.iter(|| format_id(&mut string, &formatter))
    });
    c.bench_function("format role id", |b| {
        let mut string = String::new();
        let formatter = Id::<RoleMarker>::new_checked(999_999_999_999_999_999).mention();

        b.iter(|| format_id(&mut string, &formatter))
    });
    c.bench_function("format user id", |b| {
        let mut string = String::new();
        let formatter = Id::<UserMarker>::new_checked(999_999_999_999_999_999).mention();
