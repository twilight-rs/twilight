...
///
/// // later in the application...
/// let guild_id = Id::new_checked(1).expect("non zero id");
...
///
/// let channel_id = ChannelId::new_checked(1).expect("non zero id");
///
/// let cache = InMemoryCache::new();
...
mod tests {
    ...
    fn test_iter() {
        ...
        let users = &[
            (Id::new_checked(2).expect("non zero"), Some(guild_id)),
            (Id::new_checked(3).expect("non zero"), Some(guild_id)),
            (Id::new_checked(4).expect("non zero"), None),
        ];
        let cache = InMemoryCache::new();
