...
mod tests {
    ...
    fn test_message_create() {
        ...
        assert_eq!(
            cache
                .member(Id::new_checked(1), Id::new_checked(3))
                .unwrap()
                .user_id,
            Id::new_checked(3),
        );
        {
            let entry = cache.channel_messages.get(&Id::new_checked(2)).unwrap();
            assert_eq!(entry.value().len(), 2);
        }

        let mut iter = cache
            .channel_messages(Id::new_checked(2))
