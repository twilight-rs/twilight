...
mod tests {
    ...
    fn test_insert_role_on_event() {
        ...
        cache.update(&RoleCreate {
            ...
            role: test::role(Id::new_checked(2)),
        });

        {
            assert_eq!(1, cache.guild_roles.get(&Id::new_checked(1)).unwrap().len());
            assert_eq!(1, cache.roles.len());

            assert_eq!(
                "test".to_string(),
                cache.role(Id::new_checked(2)).unwrap().name
            );
        }
    }
    ...
    fn test_cache_role() {
        ...
        // Single inserts
        {
            ...
            let guild_1_role_ids = (1..=10).map(Id::new_checked).collect::<Vec<_>>();
            ...
            for role in guild_1_roles.clone() {
                cache.cache_role(Id::new_checked(1), role);
            }

            // Check for the cached guild role ids
            let cached_roles = cache.guild_roles(Id::new_checked(1)).unwrap();
        ...
        // Bulk inserts
        {
            ...
            let guild_2_role_ids = (101..=110).map(Id::new_checked).collect::<Vec<_>>();
            // Map the role ids to a test role
            ...
            cache.cache_roles(Id::new_checked(2), guild_2_roles.clone());

            // Check for the cached guild role ids
            let cached_roles = cache.guild_roles(Id::new_checked(2)).unwrap();
