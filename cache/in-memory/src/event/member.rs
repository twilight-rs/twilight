...
mod tests {
    ...
    fn test_cache_guild_member() {
        ...
        // Single inserts
        {
            let guild_1_user_ids = (1..=10).map(Id::new_checked).collect::<Vec<_>>();
            let guild_1_members = guild_1_user_ids
                .iter()
                .copied()
                .map(|id| test::member(id, Id::new_checked(1)))
                .collect::<Vec<_>>();

            for member in guild_1_members {
                cache.cache_member(Id::new_checked(1), member);
            }

            // Check for the cached guild members ids
            let cached_roles = cache.guild_members(Id::new_checked(1)).unwrap();
            ...
            assert!(guild_1_user_ids
                .iter()
                .all(|id| cache.member(Id::new_checked(1), *id).is_some()));
        ...
        // Bulk inserts
        {
            let guild_2_user_ids = (1..=10).map(Id::new_checked).collect::<Vec<_>>();
            let guild_2_members = guild_2_user_ids
                .iter()
                .copied()
                .map(|id| test::member(id, Id::new_checked(2)))
                .collect::<Vec<_>>();
            cache.cache_members(Id::new_checked(2), guild_2_members);
    ...
    fn test_cache_user_guild_state() {
        let user_id = Id::new_checked(2);
        let cache = InMemoryCache::new();
        cache.cache_user(Cow::Owned(test::user(user_id)), Some(Id::new_checked(1)));

        // Test the guild's ID is the only one in the user's set of guilds.
        {
            let user_guilds = cache.user_guilds.get(&user_id).unwrap();
            assert!(user_guilds.contains(&Id::new_checked(1)));
            assert_eq!(1, user_guilds.len());
        }

        // Test that a second guild will cause 2 in the set.
        cache.cache_user(Cow::Owned(test::user(user_id)), Some(Id::new_checked(3)));

        {
            let user_guilds = cache.user_guilds.get(&user_id).unwrap();
            assert!(user_guilds.contains(&Id::new_checked(3)));
