...
mod tests {
    ...
    fn test_cache_guild_member() {
        ...
        // Single inserts
        {
            ...
            let guild_1_members = guild_1_user_ids
                .iter()
                .copied()
                .map(|id| test::member(id, Id::new_checked(1).expect("non zero")))
                .collect::<Vec<_>>();

            for member in guild_1_members {
                cache.cache_member(Id::new_checked(1).expect("non zero"), member);
            }
        ...
        // Bulk inserts
        {
            ...
            let guild_2_members = guild_2_user_ids
                .iter()
                .copied()
                .map(|id| test::member(id, Id::new_checked(2).expect("non zero")))
                .collect::<Vec<_>>();
    ...
    fn test_cache_user_guild_state() {
        ...
        cache.cache_user(
            Cow::Owned(test::user(user_id)),
            Some(Id::new_checked(3).expect("non zero")),
        );

        {
            let user_guilds = cache.user_guilds.get(&user_id).unwrap();
            assert!(user_guilds.contains(&Id::new_checked(3).expect("non zero")));
        ...
        cache.update(&MemberRemove {
            guild_id: Id::new_checked(3).expect("non zero"),
