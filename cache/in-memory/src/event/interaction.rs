...
mod tests {
    ...
    fn test_interaction_create() {
        ...
        {
            let guild_members = cache.guild_members(Id::new_checked(3)).unwrap();
            assert_eq!(guild_members.len(), 2);
        }

        {
            let member = cache
                .member(Id::new_checked(3), Id::new_checked(6))
                .unwrap();
        ...
        {
            let member = cache
                .member(Id::new_checked(3), Id::new_checked(7))
                .unwrap();
            let user = cache.user(member.user_id).unwrap();
        ...
        {
            let guild_roles = cache.guild_roles(Id::new_checked(3)).unwrap();
