...
mod tests {
    ...
    fn test_guild_create_channels_have_guild_ids() -> Result<(), TimestampParseError> {
        ...
        let channels = Vec::from([GuildChannel::Text(TextChannel {
            id: Id::new_checked(111).expect("non zero"),
            guild_id: None,
        ...
        let threads = Vec::from([GuildChannel::PublicThread(PublicThread {
            id: Id::new_checked(222).expect("non zero"),
            ...
            member: Some(ThreadMember {
                flags: 0,
                id: Some(Id::new_checked(1).expect("non zero")),
                join_timestamp: timestamp,
                member: None,
                presence: None,
                user_id: Some(Id::new_checked(2).expect("non zero")),
            }),
        })]);
        ...
        let channel = cache
            .guild_channel(Id::new_checked(111).expect("non zero"))
            .unwrap();

        let thread = cache
            .guild_channel(Id::new_checked(222).expect("non zero"))
            .unwrap();
        ...
        match channel.resource() {
            GuildChannel::Text(ref c) => {
                assert_eq!(Some(Id::new_checked(123).expect("non zero")), c.guild_id);
            }
            _ => panic!("{:?}", channel),
        }
    ...
    fn test_guild_update() {
        ...
        let guild = Guild {
            ...
            owner_id: Id::new_checked(1).expect("non zero"),
            owner: None,
            permissions: None,
        ...
        let mutation = PartialGuild {
            ...
            nsfw_level: guild.nsfw_level,
            owner_id: Id::new_checked(2).expect("non zero"),
            owner: guild.owner,
            permissions: guild.permissions,
            preferred_locale: guild.preferred_locale,
            premium_subscription_count: guild.premium_subscription_count,
