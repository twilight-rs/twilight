...
mod tests {
    ...
    fn test_cache_emoji() {
        ...
        fn user_mod(id: Id<EmojiMarker>) -> Option<User> {
            if id.get() % 2 == 0 {
                ...
                Some(test::user(Id::new_checked(1).expect("non zero")))
            } else {
                None
            }
        }
        ...
        // Single inserts
        {
            let guild_1_emoji_ids = (1..=10)
                .map(|n| Id::new_checked(n).expect("non zero"))
                .collect::<Vec<_>>();
            ...
            for emoji in guild_1_emoji {
                cache.cache_emoji(Id::new_checked(1).expect("non zero"), emoji);
            }
            ...
            let guild_emojis = cache.guild_emojis(Id::new_checked(1).expect("non zero"));
        ...
        // Bulk inserts
        {
            ...
            let guild_emojis = cache.guild_emojis(Id::new_checked(2).expect("non zero"));
    ...
    fn test_emoji_removal() {
        ...
        let emote = test::emoji(Id::new_checked(1).expect("non zero"), None);
        let emote_2 = test::emoji(Id::new_checked(2).expect("non zero"), None);
        let emote_3 = test::emoji(Id::new_checked(3).expect("non zero"), None);
        ...
        let emote_4 = test::emoji(Id::new_checked(4).expect("non zero"), None);
