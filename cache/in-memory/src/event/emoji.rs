...
mod tests {
    ...
    fn test_cache_emoji() {
        ...
        // Single inserts
        {
            let guild_1_emoji_ids = (1..=10).map(Id::new_checked).collect::<Vec<_>>();
            ...
            for emoji in guild_1_emoji {
                cache.cache_emoji(Id::new_checked(1), emoji);
            }
            ...
            let guild_emojis = cache.guild_emojis(Id::new_checked(1));
        ...
        // Bulk inserts
        {
            let guild_2_emoji_ids = (11..=20).map(Id::new_checked).collect::<Vec<_>>();
            let guild_2_emojis = guild_2_emoji_ids
                .iter()
                .copied()
                .map(|id| test::emoji(id, user_mod(id)))
                .collect::<Vec<_>>();
            cache.cache_emojis(Id::new_checked(2), guild_2_emojis);
            ...
            let guild_emojis = cache.guild_emojis(Id::new_checked(2));
    ...
    fn test_emoji_removal() {
        ...
        let emote = test::emoji(Id::new_checked(1), None);
        let emote_2 = test::emoji(Id::new_checked(2), None);
        let emote_3 = test::emoji(Id::new_checked(3), None);
        ...
        let emote_4 = test::emoji(Id::new_checked(4), None);
