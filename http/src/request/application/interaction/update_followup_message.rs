...
///     .update_followup_message("token here", Id::new_checked(2).expect("non zero"))
...
impl<'a> UpdateFollowupMessage<'a> {
    ...
    /// client
    ///     .interaction(application_id)
    ///     .update_followup_message("token", Id::new_checked(2).expect("non zero"))
    ///     .embeds(Some(&[embed]))?
    ///     .exec()
    ///     .await?;
    /// # Ok(()) }
...
mod tests {
    ...
    fn test_update_followup_message() -> Result<(), Box<dyn Error>> {
        let application_id = Id::new_checked(1).expect("non zero id");
        let message_id = Id::new_checked(2).expect("non zero id");
