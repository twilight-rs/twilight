...
impl<'a> CreateInvite<'a> {
    ...
    /// let invite = client.create_invite(Id::new_checked(1))
    ///     .max_age(60 * 60)?
    ///     .exec()
    ///     .await?
    ///     .model()
...
mod tests {
    ...
    fn test_max_age() -> Result<(), Box<dyn Error>> {
        ...
        let mut builder = CreateInvite::new(&client, Id::new_checked(1)).max_age(0)?;
    ...
    fn test_max_uses() -> Result<(), Box<dyn Error>> {
        let client = Client::new("foo".to_owned());
        let mut builder = CreateInvite::new(&client, Id::new_checked(1)).max_uses(0)?;
