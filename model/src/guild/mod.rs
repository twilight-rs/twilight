                ...
                loop {
                    ...
                    match key {
                        ...
                        Field::Members => {
                            ...
                            let deserializer =
                                MemberListDeserializer::new(Id::new_checked(1).expect("non zero"));
...
mod tests {
    ...
    fn test_guild() -> Result<(), TimestampParseError> {
        ...
        let value = Guild {
            afk_channel_id: Some(Id::new_checked(2).expect("non zero")),
            afk_timeout: 900,
            application_id: Some(Id::new_checked(3).expect("non zero")),
            ...
            owner_id: Id::new_checked(5).expect("non zero"),
            owner: Some(false),
            ...
            presences: Vec::new(),
            roles: Vec::new(),
            rules_channel_id: Some(Id::new_checked(6).expect("non zero")),
            splash: Some("splash hash".to_owned()),
            stage_instances: Vec::new(),
            stickers: Vec::new(),
            system_channel_flags: SystemChannelFlags::SUPPRESS_PREMIUM_SUBSCRIPTIONS,
            system_channel_id: Some(Id::new_checked(7).expect("non zero")),
            threads: Vec::new(),
            unavailable: false,
            vanity_url_code: Some("twilight".to_owned()),
            verification_level: VerificationLevel::Medium,
            voice_states: Vec::new(),
            widget_channel_id: Some(Id::new_checked(8).expect("non zero")),
