    ...
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        let kind = match data.kind {
            PermissionOverwriteTargetType::Member => {
                let id = Id::new_checked(data.id).expect("non zero");
                #[cfg(feature = "tracing")]
...
mod tests {
    ...
    fn test_overwrite() {
        let overwrite = PermissionOverwrite {
            ...
            kind: PermissionOverwriteType::Member(Id::new_checked(12_345_678).expect("non zero")),
        };
  ...
  "type": 1
        ...
        let value = PermissionOverwrite {
            allow: Permissions::CREATE_INVITE,
            deny: Permissions::KICK_MEMBERS,
            kind: PermissionOverwriteType::Member(Id::new_checked(1).expect("non zero")),
        };
