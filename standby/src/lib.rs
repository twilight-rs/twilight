...
impl Standby {
    ...
    /// let standby = Standby::new();
    ///
    /// let guild_id = Id::new_checked(123).expect("non zero");
    ///
    ...
    /// let standby = Standby::new();
    ///
    /// let author_id = Id::new_checked(456).expect("non zero");
    /// let channel_id = Id::new_checked(123).expect("non zero");
    ///
    ...
    /// let standby = Standby::new();
    ///
    /// let message_id = Id::new_checked(123).expect("non zero");
    /// let user_id = Id::new_checked(456).expect("non zero");
    ///
    ...
    ///     event.author_id() == Some(Id::new_checked(456).expect("non zero"))
    /// }).await?;
    /// # Ok(()) }
    /// ```
    ///
...
mod tests {
    ...
    fn message() -> Message {
        Message {
            ...
            embeds: Vec::new(),
            flags: None,
            guild_id: Some(Id::new_checked(4).expect("non zero")),
            interaction: None,
    ...
    fn reaction() -> Reaction {
        Reaction {
            channel_id: Id::new_checked(2).expect("non zero"),
            emoji: ReactionType::Unicode {
                name: "🍎".to_owned(),
            },
            guild_id: Some(Id::new_checked(1).expect("non zero")),
            member: None,
            message_id: Id::new_checked(4).expect("non zero"),
    ...
    fn button() -> MessageComponentInteraction {
        MessageComponentInteraction {
            application_id: Id::new_checked(1).expect("non zero"),
            channel_id: Id::new_checked(2).expect("non zero"),
            ...
            guild_id: Some(Id::new_checked(3).expect("non zero")),
    ...
    async fn test_matched() {
        ...
        let standby = Standby::new();
        let guild_id_one = Id::new_checked(1).expect("non zero");
        let guild_id_two = Id::new_checked(2).expect("non zero");
    ...
    async fn test_wait_for_stream() {
        ...
        standby.process(&Event::RoleDelete(RoleDelete {
            guild_id: Id::new_checked(1).expect("non zero"),
            role_id: Id::new_checked(3).expect("non zero"),
        }));
        ...
        standby.process(&Event::RoleDelete(RoleDelete {
            guild_id: Id::new_checked(1).expect("non zero"),
            role_id: Id::new_checked(4).expect("non zero"),
        }));
        assert!(standby.guilds.is_empty());
    }
    ...
    async fn test_wait_for_component() {
        ...
        let wait = standby.wait_for_component(
            ...
            |button: &MessageComponentInteraction| {
                button.author_id() == Some(Id::new_checked(2).expect("non zero"))
            },
        );
        ...
        assert_eq!(
            Some(Id::new_checked(2).expect("non zero")),
    ...
    async fn test_process_nonspecific_handling() {
        ...
        let wait = standby.wait_for(Id::new_checked(1).expect("non zero"), |event: &Event| {
